import { DEFAULT_CATEGORIES, type MoodCategory, type StoredModel } from "../../types";
...
function orderedCategoriesPresent(categories: MoodCategory[]): MoodCategory[] {
...
export async function trainModelForDog(
  ...
  const samples = await samplesStore.byDog(dogId);
  if (samples.length < MIN_SAMPLES_TO_TRAIN) {
    throw new InsufficientDataError(
      `Need at least ${MIN_SAMPLES_TO_TRAIN} labeled samples to train (have ${samples.length}).`,
    );
  }
  ...
  if (categories.length < MIN_CATEGORIES_TO_TRAIN) {
    throw new InsufficientDataError(
      `Need samples in at least ${MIN_CATEGORIES_TO_TRAIN} different categories to train.`,
    );
  }

  const vectors = samples.map((s) => buildFeatureVector(s.features, s.postureTags));
